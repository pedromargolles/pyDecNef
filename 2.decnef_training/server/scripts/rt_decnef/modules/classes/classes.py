...
class Vol(Exp):
    def __init__(self, 
        ...
        self.in_HRF_peak = False # Whether volume is within last trial H.R.F. peak or not
...
class Trial(Exp):
    def __init__(self, 
        ...
        self.HRF_peak_vols = [] # Volumes within H.R.F. peak
        self.n_HRF_peak_vols = 0 # Number of volumes in H.R.F. peak
        self.HRF_peak_end = False # To track whether H.R.F. peak ends with the last volume appended to this trial or should wait
        self.decoding_prob = None # Decoding probability at trial level
        self.decoding_time = None # Time to decode this trial H.R.F. peak volumes
    ...
    def assign(self, vol):
        ...
        if self.trial_idx != None: # If task volume
            ...
            if self.HRF_peak_onset <= vol.vol_vs_trial_onset <= self.HRF_peak_offset: # If this volume is within H.R.F. peak thresholds append also to a specific list for decoding
                vol.in_HRF_peak = True # This volume is within H.R.F. peak
                self.HRF_peak_vols.append(vol) # Append to H.R.F. peak vols list in this Trial instance
                self.n_HRF_peak_vols = len(self.HRF_peak_vols) # Refresh number of volumes within H.R.F. peak

            if (vol.vol_vs_trial_onset + self.TR) > self.HRF_peak_offset: # Predict whether next volume will fall in H.R.F. peak or not
                self.HRF_peak_end = True # If not, collection of H.R.F. peak volumes ends
    ...
    def _decode(self):
        ...
        if self.decoding_procedure == 'average_probs':
            while (self.HRF_peak_end == False) or (self.HRF_peak_vols[-1].prepared_4_decoding == False): # If decoding signal is received before H.R.F. peak ends or last H.R.F. peak vol is prepared 
            ...
            preproc_vols_data = [vol.data for vol in self.HRF_peak_vols] # Get data arrays from every volume within H.R.F. peak
            trial_decoding_prob, vols_decoding_probs, trial_decoding_time = average_probs_decoding(preproc_vols_data = preproc_vols_data, # Masked and preprocessed volumes in H.R.F. peak (list of numpy arrays. Each array has following dimensions: 1, n_voxels)
            ...
            for vol, vol_decoding_prob in zip(self.HRF_peak_vols, vols_decoding_probs): # Update H.R.F. peak volumes objects with decoding information
            ...
            preproc_vols_data = [vol.data for vol in self.HRF_peak_vols] # Get data arrays from every volume within H.R.F. peak
            trial_decoding_prob, trial_decoding_time = average_HRF_peak_vols_decoding(preproc_vols_data = preproc_vols_data, # Masked and preprocessed volumes in H.R.F. peak (list of numpy arrays. Each array has following dimensions: 1, n_voxels)
                                                                                      model_file = self.model_file, # Pre-trained decoder
                                                                                      ground_truth = self.ground_truth, # Ground truth label to select target decoding probability (ex., 0, 1, 2...)
                                                                                     )
            ...
            while self.HRF_peak_end == False: # While H.R.F. peak has not ended
                if (len(self.HRF_peak_vols) >= 1) and (self.HRF_peak_vols[-1].prepared_4_decoding == True): # If some volume is already processed and prepared_4_decoding
                    if id(last_decoded_vol) != id(self.HRF_peak_vols[-1]): # If a new preprocessed and non decoded vol is detected within 
                        ...
                        last_vol = self.HRF_peak_vols[-1] # Get data array from last volume within H.R.F. peak

                        vol_decoding_prob, vol_decoding_time = dynamic_decoding(last_vol.data, # Last preprocessed volume within H.R.F. peak array data
